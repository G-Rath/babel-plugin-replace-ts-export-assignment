 ...
 *  - Remove `git+` from the start of the url, if present, and
 *  - Replacing `ssh://git@` with `https://`, if present, and
 *  - Removing `.git` from the end of the url, if present.
 *
 * Example #1:
 *
 * `ssh://git@bitbucket.org/organisation/repo.git`
 *
 * would become
 *
 * `https://bitbucket.org/organisation/repo`.
 *
 * @param {string} gitRepoUrl
 *
 * @return {string}
 */
const ensureProperGitRepoLink = (gitRepoUrl: string): string => gitRepoUrl
  .replace(/^git\+/, '')
 ...
 * @return {string}
 *
 * @throws {Error} when the spacer can't be determined
 */
const getCompareSpacerFromRepoHostUrl = (repoUrl: string): string => {
 ...
 * @return {string}
 *
 * @throws {Error} when the repo host isn't supported
 */
 ...
 * @throws {Error} when the repo host isn't supported
 */
const getOldVersionTagFromUnreleasedLink = (
  unreleasedLinkUrl: string
): string => {
 ...
 * @return {string}
 *
 * @throws {Error} when the compare url can't be found in the changelog
 */
const getUnreleasedCompareUrl = (
  changelog: string,
  gitUrl: string
): string => {
 ...
 */
export const updateChangelogTagLinksForNewVersion = (
